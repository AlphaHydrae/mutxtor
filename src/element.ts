...
export default class TextElement extends TextRange {
   ...
   *
   * @param document The document in which the element was found or is to be
   * inserted.
   * @param start The index at which the element's text is found in the
   * document.
   * @param text The element's text.
   */
  constructor(document: TextDocument, start: number, text: string) {
    // TODO: validate
   ...
   * The supplied {@link TextDocument} has many utility methods to insert,
   * modify or remove text.
   *
   * @returns A promise which will be resolved when the mutation is complete, or
   * nothing if the mutation was synchronous.
   */
  mutate() {
    // nothing to do
  }
  ...
  /**
   * Prepends text before this element. The text is not included into the
   * element. Rather, the element is moved forward in the document to take into
   * account the added text.
   *
   * Utility method for use in {@link TextElement#mutate}.
   *
   ...
   *
   * Listeners registered with this method can later be removed with
   * {@link TextElement#removeDocumentListeners}.
   *
   * @param event The name of the event to listen to.
   * @param listener The callback function.
   * @param context The context to which the callback function will be bound
   * (`this` by default).
   * @returns This text element.
   */
  onDocument(event: string, listener: (arg: any) => void, context?: any) {
   ...
   *
   * @param event If specified, all the listeners for that event will be
   * unregistered. Otherwise, all listeners for all events will be unregistered.
   * @returns This text element.
   */
  removeDocumentListeners(event?: string) {
  ...
  /**
   * Called when this element's document is mutated. If the mutation took place
   * before this element in the text, this method automatically moves the
   * element's start and end indices by the correct amount.
   *
   * If the mutation concerns this element, this method does nothing (it is the
   * responsibility of elements to mutate themselves; this method only reacts to
   * the mutation of other elements).
   *
   * @param {TextMutation} mutation - The mutation that occurred.
   * @returns {undefined} Nothing.
   */
  onDocumentMutated(mutation: TextMutation) {
